import React, { useId, useState, useContext, useMemo } from "react";
...
async function formatProducts() {
  const products = await user.getLiveProducts();
...
export default function ProductsList({
  ...
  const filteredProducts = useMemo(() => {
    let existingProducts = formatProducts();
    let new_products = [];
    if (
      (searchProduct != null || searchProduct !== "") &&
      searchProduct.length > 0
    ) {
      new_products = existingProducts.filter((product) =>
      ...
      if (
        ...
        new_products = existingProducts;
      } else {
        new_products = existingProducts.filter(
            ...
            <div className="card" style={{ width: "18rem" }}>
              ...
              <div className="card-body">
                <div className="card-title">
                  <span className="text-normal"> {product.product_name} </span>{" "}
                  <br />
...
export function getProduct(productNumber) {
  return formatProducts().find((eProduct) => {
    console.log(
